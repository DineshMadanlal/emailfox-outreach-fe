...
// Components
import SelectMailbox from 'components/Dropdown/SelectMailbox.vue';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const disableSendButton = computed(() => {
      ...
      if (isEmpty(state.fromEmailJson)) return true;
    ...
    const sendReplyEmail = async (htmlContent) => {
      try {
        ...
        const payload = {
          email: state.fromEmailJson.email,
          domain_id: state.fromEmailJson.domain_id,
        ...
        // API Call
        await postApiCall({
          endpoint: '/master-inbox/messages/compose',
          payload,
        });
